...
import { getTTListingData } from "../services/api"; // mock API call
...
const TTListingPage = () => {
  ...
  const handleApprove = async (item) => {
    try {
      const response = await axios.put(
        `${API_URL}/api/TTInvoiceApproval/approval1?approval=${"1"}&createdby=${localStorage.getItem(
          "userName"
  ...
  const handleReject = async (item) => {
    try {
      const response = await axios.put(
        `${API_URL}/api/TTInvoiceApproval/approval1?approval=${"0"}&createdby=${localStorage.getItem(
          "userName"
        )}&id=${parseInt(item.id)}&userType=${localStorage.getItem("userType")}`
      );
  ...
  const approvedList = () => {
    navigate("/TTApprovedList"); // Navigate to the approved list page
  };
          ...
          <Col xs={24} sm={16} md={22} lg={22}>
            <Card
              title={
                ...
                >
                  <ButtonTrans />
                  <p>TT - Additional Credit Requests</p>
                  <div>
                    <Button
                      className="button1"
                      type="text"
